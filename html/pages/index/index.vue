...
<script>
	...
	export default {
		...
		data() {
			return {
				dataList: [],			//播放列表
				audio: ['https://www.youthsweet.com/file/危险派对.mp3'],
				now: 0,					//默认播放歌曲
		...
		methods: {
			...
			flush_data(r){
				uni.request({
					url:'https://www.youthsweet.com/' + this.search_type + '/api?keyword=' + r,
					method:'GET',
					data:{},
					success: (res) => {
						this.now = 0
			...
			get_list(s){
				uni.request({
					url:'https://www.youthsweet.com/netease/api?keyword=' + s,
					method:'GET',
					data:{},
					success: (res) => {
						this.dataList = res.data
		...
		beforeCreate() {
			uni.request({
				url:'https://www.youthsweet.com/netease/api?keyword=' + "抖音",
				method:'GET',
				data:{},
				success: (res) => {
					this.dataList = res.data
